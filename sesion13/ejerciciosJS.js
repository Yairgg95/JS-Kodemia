const movies = [
  ...
  // Ejercicios:Crear funciones que cumplan con los siguientes requerimientos:
    ...
    // 1. Obtener una lista de los géneros sin repetir
     ...
     const getGenerList = (list) => {
    let result = list.reduce((accum, current) =>  accum.includes(current.genre) ? accum : [...accum, current.genre],[]);
    return result;
    }
  
    console.log(getGenerList(movies));


     //2. Obtener una lista de los nombres de los directores sin repetir

     const getDirectorList = (list) => {
        let result = list.reduce((accum, current) =>  accum.includes(current.director) ? accum : [...accum, current.director],[]);
        return result;
        }
      
        console.log(getDirectorList(movies));
     ...
     const getRatingList = (list) => {
        let result = list.reduce((accum, current) =>  accum.includes(current.rating) ? accum : [...accum, current.rating],[]);
        return result;
        }
